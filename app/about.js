...
export default function About(){
    ...
    return (
    <div className='relative min-h-screen w-screen bg-neutral-800 z-0 flex flex-row justify-center font-rubik overflow-x-hidden'>
        <div className="absolute w-full h-full"></div>
        <div className="absolute top-1/2 " ref={ref}></div>
        <div className=' absolute -top-4ç w-full h-8 backdrop-blur-md z-10'></div>
        ...
        <div className="transition-all delay-300 duration-2000 absolute w-40vh h-70vh bg-neutral-400 self-center right-72 -scale-x-100 md:hidden    sm:hidden" style={chevron("right")}></div>
            ...
            <div 
            className='self-center relative transition-opacity duration-700 delay-700 flex flex-col w-[33%] md:w-[80%] h-fit p-10 sm:py-0 sm:p-4 m-10 sm:m-0 justify-start border-4 border-neutral-400 bg-neutral-300'
            style = {activated ? {'borderOpacity':1} : {'borderColor':'rgba(0,0,0,0)', color:'rgba(0,0,0,.3)'}}
            >
                <Image src={rowanphoto} className="border-2 self-center drop-shadow-sm border-neutral-400 md:w-max md:h-auto m-10 sm:mt-4"/>
